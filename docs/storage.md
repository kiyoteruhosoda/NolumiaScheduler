...
選べます。既定は **JSON** です。切り替えはコード変更なしで、**設定ファイル**で行います。

- 切り替え: `storage.json` の `backend`（`Json` / `Sqlite`、既定 `Json`）。
  管理 CLI の `set-backend` コマンドで設定します。
- 既存データの移行: 管理 CLI の `migrate`（`json-to-sqlite` / `sqlite-to-json`）。

選択は合成ルート（`NolumiaScheduler.WinUI/App.xaml.cs`）が `storage.json` から読み取り、
ドメインのリポジトリインターフェース（`ICalendarEventRepository` ほか）に対して実装だけが
...
| バックエンド | 場所 |
|---|---|
| 共通 | `storage.json`（どのバックエンドを使うかの選択。バックエンド非依存） |
...
管理 CLI で `storage.json` の値を設定します（環境変数は使いません）。

```powershell
# SQLite に切り替え（再起動で反映）
dotnet run --project NolumiaScheduler.Cli -- set-backend sqlite

# JSON（既定）に戻す
dotnet run --project NolumiaScheduler.Cli -- set-backend json

# 現在の選択を確認
dotnet run --project NolumiaScheduler.Cli -- config
```

`storage.json` が無い／値が不正な場合は `Json` にフォールバックします。
切り替えはアプリ起動時に読み込まれるため、**実行中のアプリは再起動**してください。
...
# 4. アプリを SQLite で起動するよう選択（storage.json に書き込み。再起動で反映）
dotnet run --project NolumiaScheduler.Cli -- set-backend sqlite
```
...
> バックエンドの**選択**（アプリ起動時にどちらを使うか）は `storage.json`
> （[2 章](#2-バックエンドの切り替え)、`set-backend` で設定）です。**データの移行**は
> `migrate` です。どちらも管理 CLI で操作します。
